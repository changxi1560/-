    ...
    while (true) {
        if (isCross == 0) {  // 根据红外传感器的输出来决定运动模式
            ...
            } else if (isAllHigh() && isFinish == 0)  // 全为黑线时，到达终点
            {
                ...
                // turnAroundandDrop();
                return SLOW_ON;
            } else if (CTRTstate[7][0] == HIGH)  // 直角右转
            {
                return TURN_RIGHT_HIGH_F;
            } else if (CTRTstate[6][0] == HIGH)  // 中右转
            {
                return TURN_RIGHT_MID;
            ...
            } else if (CTRTstate[5][0] == HIGH)  // 低右转
            {
                return TURN_RIGHT_LOW;
            } else if (CTRTstate[3][0] == HIGH || CTRTstate[4][0] == HIGH)  // 直走
            {
                return STRAIGHT_ON;
            } else  // 直走
            {
        ...
        if (isCross == 1) {  // 记忆模式
            ...
            */
            ...
            if (CTRTstate[0][1] == HIGH || CTRTstate[0][2] == HIGH || CTRTstate[0][3] == HIGH)  // 直角左转
            {
                quarter_turn = QT_L;
                return TURN_LEFT_HIGH_F;
            } else if (CTRTstate[7][1] == HIGH || CTRTstate[7][2] == HIGH || CTRTstate[7][3] == HIGH)  // 直角右转
            {
                quarter_turn = QT_R;
                return TURN_RIGHT_HIGH_F;
            } else if (CTRTstate[1][1] == HIGH || CTRTstate[1][2] == HIGH || CTRTstate[1][3] == HIGH)  // 中左转
            {
                // return TURN_LEFT_MID;
                return TURN_LEFT_HIGH;
            } else if (CTRTstate[6][1] == HIGH || CTRTstate[6][2] == HIGH || CTRTstate[6][3] == HIGH)  // 中右转
            {
                // return TURN_RIGHT_MID;
                return TURN_RIGHT_HIGH;
            } else if (CTRTstate[2][1] == HIGH || CTRTstate[2][2] == HIGH || CTRTstate[2][3] == HIGH)  // 低左转
            {
                // return TURN_LEFT_LOW;
                return TURN_LEFT_HIGH;
            } else if (CTRTstate[5][1] == HIGH || CTRTstate[5][2] == HIGH || CTRTstate[5][3] == HIGH)  // 低右转
            {
                // return TURN_RIGHT_LOW;
                return TURN_RIGHT_HIGH;
            } else {
    ...
    */
    switch (mode) {
        ...
        case TURN_LEFT_LOW:  // 低左转
            ...
            TARGET_V_RIGHT = -base_V * 1 ;  // 0.9
            break;
        case TURN_LEFT_MID:  // 中左转
            TARGET_V_LEFT = base_V * 0.2;
            TARGET_V_RIGHT = -base_V ;
            break;
        case TURN_LEFT_HIGH:  // 直角左转
            TARGET_V_LEFT = -base_V*0.3;
            TARGET_V_RIGHT = -base_V*1.25;
            break;
        case TURN_LEFT_HIGH_F:  // 直角左转_快速
            TARGET_V_LEFT = -base_V * 0.5;
            TARGET_V_RIGHT = -base_V * 1.7;
            break;
